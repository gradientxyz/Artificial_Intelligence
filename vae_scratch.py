...
class VAE(nn.Module):
    ...
    def get_loss(self, x):
        ...
        L2 = - d2l.sum(1 + d2l.log(sigma ** 2) - mu ** 2 - sigma ** 2)
...
transform = transforms.Compose([
                transforms.ToTensor(),
                transforms.Lambda(d2l.flatten) # falatten
            ])
dataset = datasets.MNIST(root='./', train=True, 
                         download=False, transform=transform)
dataloader = d2l.utils.data.DataLoader(
...
with d2l.no_grad():
    sample_size = 64
    z = d2l.randn(sample_size, latent_dim)
    x = model.decoder(z)
